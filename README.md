...
- **v1.7.3** — Hotfix: methodology page incorrectly framed X1 Strontium as "a separate X1 Labs project" in the Layer 2 explainer and References list (EN + PL). X1 Strontium is an independent open-source time oracle — not part of X1 Labs' product lineup. Reworded four prose clauses to neutral framing ("X1 Strontium — an independent time oracle on X1 mainnet"). No technical description changed, no widget or chart overlay touched. Frontend (docs only).
- **v1.7.2** — Five chart-polish fixes after Theo's 15.05 walkthrough. (1) `chart-history` toolbar gains `30d` and `90d` buttons (window now `2 / 4 / 6 / 12 / 30 / 90`); plus new `60-min` and `3h` bucket-aggregation levels to keep the chart density readable. (2) `chart-foundation-trend` gets a `7d / 14d / 30d` selector (default `14d` — zero regression); stat-card numbers (current, 7d-delta, operational alert) keep operating on the full retained dataset so long-term signals don't shift with window changes. (3) Adaptive X-axis tick density per window — `2d` shows `HH:mm` with 6 ticks, `4d` shows `MM-DD HH:mm`, `6d+` shows `MM-DD`; tooltip always shows the full ISO timestamp regardless of axis abbreviation. (4) Strontium widget header `🕐` emoji replaced with an inline SVG clock that inherits `currentColor`, so the icon renders deterministically across font stacks (the deployed dashboard was falling back to `⚪`). (5) General polish: Y-axis `grace: 10%` on all four charts (auto-ranged scales gain breathing room; outlier-clamped scales unchanged), unified tooltip styling across the four charts, and a `@media (max-width: 600px)` block tightens chart frame heights + button padding + helper text on mobile. New EN+PL i18n key `chart_history_fallback_hint`. Frontend only.
- **v1.7.1** — Two small chart polishes. (1) Network-drift chart Y-axis title now renders explicitly with the same slate-400 / 11 px style as the foundation-trend and scatter charts (was previously rendering at the default font size, occasionally clipped on narrow viewports). (2) Strontium oracle overlay on all three ms-scale charts (`chart-history`, `chart-foundation-trend`, `chart-scatter`): amber dashed reference line at `y = 0` labeled "Strontium ref", legend row "🔬 Strontium active — spread: Xms conf: Y%", and tooltip footer line on hover. Strontium spread/confidence values are NOT plotted on the pipeline-lag y-axis (different units / meaning). Charts gracefully render unchanged when `strontium.json` is unavailable. Frontend-only.
- **v1.7.0** — X1 Strontium Oracle widget on the dashboard. New `install/strontium-to-json.sh` parses `x1sr read` plain-text output into `data/strontium.json` (cron-driven on Sentinel, independent of the ClockDrift daemon). The daemon's existing `git add data/` push picks it up automatically; a new tracing line confirms its presence + age each cycle. Frontend gets a compact panel showing last timestamp, spread (with confidence-bar), confidence %, slot, oracle PDA, and a 288-point spread sparkline. Hidden silently if `strontium.json` is missing or fetch fails — no error UX. EN+PL i18n. No changes to `summary.json`, `pipeline_anomalies.json`, `isActiveValidator()`, or the Layer 1 / Layer 2 framework.
- v1.6.0 — Three communication fixes (Theo feedback). Bug #1: `pipeline_anomalies.json` now filters out `severity = "healthy"` and entries within ±200 ms of the −540 ms foundation baseline (no more "anomaly file with healthy entries"). Bug #2: UI terminology — "drift" replaced by "pipeline lag" in Layer 1 contexts (ms-scale), "drift" retained for Layer 2 (≥5 s clock skew). EN + PL i18n strings + HTML placeholders updated. Bug #3: network-drift chart gains a green "Normal range" band (`[−740, −340]` ms), a dashed `−540 ms` baseline reference line, a tooltip showing live deviation, and a `✅ NORMAL` / `⚠️ WARNING` / `🔴 ANOMALY` status badge. Frontend-only otherwise; daemon change is the SQL filter only.
- v1.5.1 — Daemon operability hardening: `idx_stake_snap_validator_ts` (50× faster stake-refresher hot query), non-blocking backfill (exporter publishes `summary.json` immediately on restart instead of waiting up to 90 minutes), backfill default lookback reduced from 7 days to 1 day (configurable via `backfill_lookback_days`), `MemoryMax=2G` / `CPUQuota=80%` in the systemd unit (sized for actual v1.5.x working set), opt-in `validate_against_rpc` cross-check against `getVoteAccounts`. Frontend untouched.
